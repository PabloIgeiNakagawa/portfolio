...
export default function TechBackground() {
  ...
  useEffect(() => {
    ...
    // Seguir mouse - usando ref para evitar re-renders
    const handleMouseMove = (e) => {
      mouseRef.current = { x: e.clientX, y: e.clientY };
      // Forzar actualización solo del efecto del mouse
      forceUpdate({});
    };
  ...
  return (
    <div className="fixed inset-0 pointer-events-none overflow-hidden">
      ...
      {/* Efecto de mouse con ondas - SIN transition para seguimiento inmediato */}
      <div
        className="absolute w-96 h-96 opacity-40 pointer-events-none"
        style={{
          left: `${mouseRef.current.x - 192}px`,
          top: `${mouseRef.current.y - 192}px`,
          transform: 'translate3d(0, 0, 0)', // Forzar aceleración por hardware
        }}
      >
        <div className="w-full h-full bg-gradient-to-r from-blue-400 to-purple-500 rounded-full blur-3xl animate-pulse" />
        <div className="absolute inset-0 w-full h-full bg-gradient-to-r from-cyan-400 to-blue-500 rounded-full blur-2xl animate-pulse opacity-60" style={{ animationDelay: '0.5s' }} />
      </div>
